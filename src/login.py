...
from src.configuration.utils import decryptString, KEY, savedLogin, ttk_style, icon
...
def Login():
    ...
    window = sg.Window('PsychoPass Login', layout, icon=icon, element_justification='center', margins=(20, 20), use_ttk_buttons=True, ttk_theme=ttk_style)
    ...
    while True:
        ...
        if event == 'Login':
            ...
            if createdAcc is True:
                ...
                print(f'(event)-decryptedString(saved_pass): {saved_pass}')
                if given_name == saved_name and given_pass == saved_pass:
                    window.close()
                    Home(key)
                    sg.popup('Either key is missing or is invalid', icon=icon)

                else:
                    sg.popup('Incorrect login', icon=icon)
            else:
                sg.popup('Please Create Login First', icon=icon)
        if event == 'Create Login':
            login_made = ReadSettings('LoginMade', JsonPath)
            if login_made is True:
                sg.popup('Login Already Created', icon=icon)
